...
public:
    struct SBillboard
    {
        ...
        SBillboard( const core::vector3df& pos, const core::dimension2df& size, f32 roll, video::SColor color )
            : Position(pos), Size(size), Roll(roll), HasAxis(false), Color(color), prev(NULL), next(NULL) {}
    };
    ...
    ~CBillboardGroupSceneNode();

	virtual ESCENE_NODE_TYPE getType() const { return (ESCENE_NODE_TYPE)MAKE_IRR_ID('b','i','l','g'); }

    //! Adds a billboard.
    SBillboard * addBillboard( const core::vector3df& position, const core::dimension2df& size, f32 roll=0.0f, video::SColor vertexColor=video::SColor(255,255,255,255) );

    //! Adds a billboard resitricted to an axis.
    SBillboard *  addBillboardWithAxis( const core::vector3df& position, const core::dimension2df& size, const core::vector3df& axis, f32 roll=0.0f, video::SColor vertexColor=video::SColor(255,255,255,255) );

    void removeBillboard( CBillboardGroupSceneNode::SBillboard * bill );
	...
	void forceUpdate( void ) { refresh = true; }
