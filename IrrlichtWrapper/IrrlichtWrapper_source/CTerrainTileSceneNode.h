...
// Converted into a tilable scene node by Frank Dodd (2009) all modifications
// distributed under the same above irrlicht copyright notice.

#ifndef __C_TERRAIN_TILE_SCENE_NODE_H__
#define __C_TERRAIN_TILE_SCENE_NODE_H__
...
#define ADJACENT_TOP	0
#define ADJACENT_BOTTOM	1
...
namespace scene
{
	...
	class CTerrainTileSceneNode : public ITerrainSceneNode
	{
	public:
		...
		CTerrainTileSceneNode(ISceneNode* parent, ISceneManager* mgr, io::IFileSystem* fs, s32 id,
			s32 maxLOD = 4, E_TERRAIN_PATCH_SIZE patchSize = ETPS_17,
			const core::vector3df& position = core::vector3df(0.0f, 0.0f, 0.0f),
			const core::vector3df& rotation = core::vector3df(0.0f, 0.0f, 0.0f),
			const core::vector3df& scale = core::vector3df(1.0f, 1.0f, 1.0f));

		virtual ~CTerrainTileSceneNode();
		...
		//! Initializes the terrain data.  Loads the vertices from a supplied heightMapFile.
        bool loadHeightMapImage(
		...
		//! Load the terrain color data from the supplied image file.
		virtual void loadColor( video::IImage* colorMap, core::position2d<s32> dataPosition );

		//! Load the heights and UV Scaling on the terrain
		virtual void loadStructure( video::IImage * structureMap, core::position2d<s32> dataPosition, s32 startRow, s32 endRow );

		//! resets the position of the node.
		void loadPosition ( const core::vector3df& newpos, s32 startRow, s32 endRow );
		...
		//! Attach an adjacent tile to this tile
		void attachTile( CTerrainTileSceneNode *tile, unsigned int edge );

		// get the size of the terrain
		s32 getTerrainSize( void ) { return TerrainData.Size; }
	...
	private:
		...
		f32 CameraFOVDelta;
		// connectivity information for adjacent tiles
		CTerrainTileSceneNode *adjacentTiles[4];
...
#endif // __C_TERRAIN_TILE_SCENE_NODE_H__
