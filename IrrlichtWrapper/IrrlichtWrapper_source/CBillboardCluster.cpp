...
//! adds an LOD cloud to the cloud system
u32 CCloudSceneNode::addCloud( u32 parent, f32 X, f32 Y, f32 Z, u32 frame )
{
	// this sets the number of top level clouds
	ParticleCount = ParticleData.size() + 1; //count;
	// ParticleData[<ParticleCount] = top level cloud data
	// ParticleData[>=ParticleCount] = child node data

    // increase the particle array for this new particle
	u32 i = ParticleData.size();
	ParticleData.set_used( i + 1 );

    ParticleData[i].pos = core::vector3d<f32>(X,Y,Z);
...
//! removes a LOD cloud and its children
void CCloudSceneNode::removeCloud( u32 index )
{
}


//! sets the position and size of a cloud
void CCloudSceneNode::setCloudPosition( u32 index, f32 X, f32 Y, f32 Z, f32 size )
{
}

//! sets the color of a cloud
void CCloudSceneNode::setCloudColor( u32 index, f32 R, f32 G, f32 B, f32 A )
{
}

//! sets the animation properties of a cloud
void CCloudSceneNode::setCloudAnimator( u32 index, f32 X, f32 Y, f32 Z, f32 R, f32 G, f32 B, f32 A, f32 size )
{
}
...
s16 CCloudSceneNode::makeChildClouds(SCloudParticle &parent,s16 start,core::matrix4 &m, core::vector3df &campos )
{
	...
	for(count=0;count<parent.children;count++)
	{
		...
		Particles[c].distance = (u32)(td * 10000.0f);
		...
		if (lod < LOD && Particles[c].children > 0)
		{
			...
			depth-=1;
			// grow buffer?
			if ((s16)Particles.size() < c+Particles[c].children)
			{
...
void CCloudSceneNode::render()
{
	...
	// draw the mesh
	driver->drawVertexPrimitiveList( Vertices.pointer(), (ParticlesToDraw-sp)*4,
		Indices.pointer(), (ParticlesToDraw-sp)*2, video::EVT_STANDARD, EPT_TRIANGLES);
...
void CCloudSceneNode::makeRandomCloud(s16 count)
{
	...
	// make particles
	for (i=1; i<(u32)count+25; i++)
	{
		f32 a,b,c;
		// random position
		if (i < (u32)count)
		{
			a = rand() / 1.5f - (rand() / 1.5f);
			c = rand() / 1.5f - (rand() / 1.5f);
			b = rand() / 50.0f - (rand() / 50.0f);
		}
		else
		{
			a = rand() / 10.0f - (rand() / 10.0f);
			c = rand() / 10.0f - (rand() / 10.0f);
			b = rand() / 10.0f - (rand() / 10.0f);
		}
