...
public:
	...
	CGrassPatchSceneNode(
        ...
        video::IImage* grassMap, scene::IWindGenerator *windgen, bool crossed );
    ...
    scene::IWindGenerator * getWind(){ return WindGen; }

    u32 getLastDrawCount() { return lastdrawcount; }

    void setFadeDistance( f32 distance ) { fadeDist = distance; }
    f32 setFadeDistance( void ) { return fadeDist; }
...
private:
    ...
    bool Load();
    bool Save();
    bool Create(core::position2d<s32> dataPosition);

    s32                           GRASS_PATCH_SIZE; // the scale of this grass patch
    f32                           grassScale;       // the scale of each grass clump
	...
	f32                           fadeDist;         // the patch scale at which particles are removed
	bool						  crossed;			// whether particles are paired and displayed as a cross
};
