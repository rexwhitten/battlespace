...
s32 debug = 0, debug2 = 0;
	...
	//! constructor
	CTerrainSphereNode::CTerrainSphereNode(ISceneNode* parent, ISceneManager* mgr,
...
//				v.X = -v.X;
	...
	bool CTerrainSphereNode::loadHeightMap( io::IReadFile** file, video::SColor vertexColor, s32 smoothFactor )
	{
	    s32 face;
 		...
 		for ( face = 0; face < CUBE_FACES; ++face )
		{
			...
			SMeshBufferLightMap* pMeshBuffer;
			pMeshBuffer = new SMeshBufferLightMap();
...
// Diagnostics for edge alignment
/*
if ( x < 5 )						// RED
{
	vertex.Color.setRed(255);
	vertex.Color.setGreen(0);
	vertex.Color.setBlue(0);
}
else if ( x > TerrainData.Size-5 )	// YELLOW
{
	vertex.Color.setRed(255);
...
{
	vertex.Color.setRed(0);
	vertex.Color.setGreen(0);
	vertex.Color.setBlue(255);
}
else if ( z > TerrainData.Size-5 )	// GREEN
{
	vertex.Color.setRed(0);
	vertex.Color.setGreen(255);
	vertex.Color.setBlue(0);
...
if (( x > 16 ) && ( x < TerrainData.Size-16 ) &&
    ( z > 16 ) && ( z < TerrainData.Size-16 ))
{
	if ( face == 1 )
	{
		vertex.Color.setRed(255);
	...
	{
		vertex.Color.setRed(128);
		vertex.Color.setGreen(128);
		vertex.Color.setBlue(128);
	}
}
else
{
...
/*
	vertex.Color.setRed(x);
	vertex.Color.setGreen(z);
	vertex.Color.setBlue(z);
*/
/*
	vertex.Color.setRed((x+z)/2);
	vertex.Color.setGreen((x+z)/2);
	vertex.Color.setBlue((x+z)/2);
*/
					/* apply the same scaling to the texture co-ordinates too */
...
*/
    ...
    void CTerrainSphereNode::setTextures (
                                            video::ITexture *textureTop,
 		...
 		for ( s32 face = 0; face < CUBE_FACES; ++face )
		{
			...
			for( s32 x = 0; x < TerrainData.Size; ++x )
			{
				for( s32 z = 0; z < TerrainData.Size; ++z )
				{
					...
					RenderBuffer[face].Vertices[index++].Color = color;
				}
			}
		}
	}
...
*/
	...
	void CTerrainSphereNode::applyTransformation()
	{
		...
		for ( s32 face = 0; face < CUBE_FACES; ++face )
		{
			...
			{
				RenderBuffer[face].Vertices[i].Pos = meshVertices[i].Pos * TerrainData.Scale + TerrainData.Position;

				RenderBuffer[face].Vertices[i].Pos -= TerrainData.RotationPivot;
				rotMatrix.inverseRotateVect( RenderBuffer[face].Vertices[i].Pos );
				RenderBuffer[face].Vertices[i].Pos += TerrainData.RotationPivot;
			}
	...
	void CTerrainSphereNode::render()
	{
		...
		for ( s32 face = 0; face < CUBE_FACES; ++face )
		{
			...
			driver->drawVertexPrimitiveList(
				RenderBuffer[face].getVertices(), RenderBuffer[face].getVertexCount(),
				RenderBuffer[face].getIndices(), IndicesToRender[face] / 3,
		...
		if (DebugDataVisible)
		{
			...
			const s32 count = TerrainData.PatchCount * TerrainData.PatchCount * CUBE_FACES;
			s32 visible = 0;
		...
		for ( s32 face = 0; face < CUBE_FACES; ++face )
		{
			...
			for (s32 x=0; x<TerrainData.Size; ++x)
			{
				...
				for (s32 z=0; z<TerrainData.Size; ++z)
				{
					...
					RenderBuffer[face].Vertices[index].TCoords.X = xval;
					RenderBuffer[face].Vertices[index].TCoords.Y = zval;
				    ...
				    switch(face)
				    {
				        case 0:
//                            px=TerrainData.Size-1-x; pz=z;
                            RenderBuffer[face].Vertices[index].TCoords.X = resBySize-1-xval;
                            RenderBuffer[face].Vertices[index].TCoords.Y = zval;
                            break;
				        case 1:
//                            px=TerrainData.Size-1-x; pz=TerrainData.Size-1-z;
                            RenderBuffer[face].Vertices[index].TCoords.X = resBySize-1-xval;
                            RenderBuffer[face].Vertices[index].TCoords.Y = resBySize-1-zval;
                            break;
				        case 2:
//                            px=x; pz=TerrainData.Size-1-z;
                            RenderBuffer[face].Vertices[index].TCoords.X = xval;
				        ...
				        case 3:
//                            px=z; pz=TerrainData.Size-1-x;
                            RenderBuffer[face].Vertices[index].TCoords.X = zval;
                            RenderBuffer[face].Vertices[index].TCoords.Y = resBySize-1-xval;
                            break;
				        case 4:
//                            px=TerrainData.Size-1-z; pz=TerrainData.Size-1-x;
                            RenderBuffer[face].Vertices[index].TCoords.X = resBySize-1-zval;
                            RenderBuffer[face].Vertices[index].TCoords.Y = resBySize-1-xval;
                            break;
				        case 5:
//                            px=x; pz=z;
...
//						RenderBuffer[face].Vertices[index].TCoords2.Y = z2val;
	...
	void CTerrainSphereNode::calculateNormals ( SMeshBufferLightMap* pMeshBuffer, s32 face )
	{
		s32 count;
			...
			for (s32 z=0; z<TerrainData.Size; ++z)
			{
				...
				if (x>0 && z>0)
				{
                    ...
                    if (( face == 1 ) || ( face == 3 ) || ( face == 5 ))
                    {
                        ...
                        a = pMeshBuffer->Vertices[x*TerrainData.Size+z].Pos;
                    }
                    else
                    {
				...
				else
				{
				    ...
				    normal = pMeshBuffer->Vertices[x*TerrainData.Size+z].Pos;
	    			count += 2;
				}
				...
				for( s32 z = 0; z < TerrainData.PatchCount; ++z )
				{
					...
					for( s32 xx = x*(TerrainData.CalcPatchSize); xx < ( x + 1 ) * TerrainData.CalcPatchSize; ++xx )
						for( s32 zz = z*(TerrainData.CalcPatchSize); zz < ( z + 1 ) * TerrainData.CalcPatchSize; ++zz )
							TerrainData.Patches[index].BoundingBox.addInternalPoint( RenderBuffer[face].Vertices[xx * TerrainData.Size + zz].Pos );
...
//			nb->loadHeightMap(file, video::SColor(255,255,255,255), 0);
		...
		for ( s32 face = 0; face < CUBE_FACES; ++face )
		{
			nb->RenderBuffer[face].Material = RenderBuffer[face].Material;
		}
