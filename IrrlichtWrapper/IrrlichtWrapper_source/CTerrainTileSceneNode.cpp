...
// The code for the TerrainTileSceneNode is based on the GeoMipMapSceneNode
...
namespace scene
{
	...
	//! constructor
	CTerrainTileSceneNode::CTerrainTileSceneNode(ISceneNode* parent, ISceneManager* mgr,
	...
	{
		#ifdef _DEBUG
		setDebugName("CTerrainTileSceneNode");
		#endif
		...
		adjacentTiles[ADJACENT_TOP] = NULL;
		adjacentTiles[ADJACENT_BOTTOM] = NULL;
		adjacentTiles[ADJACENT_LEFT] = NULL;
		adjacentTiles[ADJACENT_RIGHT] = NULL;
	}


	//! destructor
	CTerrainTileSceneNode::~CTerrainTileSceneNode()
	{
	...
	bool CTerrainTileSceneNode::loadHeightMap(io::IReadFile* file, video::SColor vertexColor,
			s32 smoothFactor)
	{
		if (!file)
			return false;
        ...
        bool result = loadHeightMap( file, vertexColor,smoothFactor );
	...
	//! Initializes the terrain data. Loads the vertices from the heightMapFile
	bool CTerrainTileSceneNode::loadHeightMapImage( video::IImage* heightMap,
...
//				vertex.Pos.Y = (f32) heightMap->getPixel(TerrainData.Size-x-1,z).getLuminance();
				vertex.Pos.Y = (f32) heightMap->getPixel(x + dataPosition.X, z + dataPosition.Y).getLuminance();
				vertex.Pos.Z = fz;
	...
	//! Initializes the terrain data. Loads the vertices from the heightMapFile
	bool CTerrainTileSceneNode::loadHeightMapRAW(io::IReadFile* file,
	...
	void CTerrainTileSceneNode::setScale(const core::vector3df& scale)
	{
	...
	void CTerrainTileSceneNode::setRotation(const core::vector3df& rotation)
	{
	...
	void CTerrainTileSceneNode::setRotationPivot(const core::vector3df& pivot)
	{
	...
	void CTerrainTileSceneNode::setPosition(const core::vector3df& newpos)
	{
	...
	void CTerrainTileSceneNode::applyTransformation()
	{
	...
	void CTerrainTileSceneNode::OnRegisterSceneNode()
	{
	...
	void CTerrainTileSceneNode::preRenderLODCalculations()
	{
	...
	void CTerrainTileSceneNode::preRenderIndicesCalculations()
	{
	...
	void CTerrainTileSceneNode::render()
	{
		if (!IsVisible || !SceneManager->getActiveCamera())
			return;
	...
	const core::aabbox3d<f32>& CTerrainTileSceneNode::getBoundingBox() const
	{
	...
	const core::aabbox3d<f32>& CTerrainTileSceneNode::getBoundingBox(s32 patchX, s32 patchZ) const
	{
	...
	void CTerrainTileSceneNode::getMeshBufferForLOD(IDynamicMeshBuffer& mb, s32 LOD ) const
	{
		if (!Mesh.getMeshBufferCount())
			return;
	...
	s32 CTerrainTileSceneNode::getIndicesForPatch(core::array<u32>& indices, s32 patchX, s32 patchZ, s32 LOD)
	{
	...
	s32 CTerrainTileSceneNode::getCurrentLODOfPatches(core::array<s32>& LODs) const
	{
		s32 numLODs;
		LODs.clear();
	...
	void CTerrainTileSceneNode::setLODOfPatch(s32 patchX, s32 patchZ, s32 LOD)
	{
	...
	//! is rendered at.
	bool CTerrainTileSceneNode::overrideLODDistance(s32 LOD, f64 newDistance)
	{
	...
	//! Colors the verticies on the terrain
	//! \param colorMap: the image file containing the color information
	//! \param dataPosition: the coordinate of the color information in the image file
	void CTerrainTileSceneNode::loadColor( video::IImage * colorMap, core::position2d<s32> dataPosition )
	{
		u32 index = 0;
		s32 inversePos = TerrainData.Size - 1;

		for ( s32 x = 0; x < TerrainData.Size; ++x )
		{
			for ( s32 z = 0; z < TerrainData.Size; ++z )
			{
			    RenderBuffer->getVertexBuffer()[index].Color.color = colorMap->getPixel(
						inversePos - (x + dataPosition.X), z + dataPosition.Y ).color;
	...
	//! Adjusts the heights and UV Scaling on the terrain
	//! \param colorMap: the image file containing the color information
	//! \param dataPosition: the coordinate of the color information in the image file
	void CTerrainTileSceneNode::loadStructure( video::IImage * structureMap, core::position2d<s32> dataPosition,
			s32 startRow, s32 endRow )
	{
		u32 index = startRow * TerrainData.Size;
		float imageStep = 1 / (float)structureMap->getDimension().Width;
		float tX = (float)dataPosition.X * imageStep;
		float tY = (float)(dataPosition.Y + startRow) * imageStep;
		s32 inversePos = TerrainData.Size - 1;

		if ( endRow > TerrainData.Size )
			endRow = TerrainData.Size;

		video::S3DVertex2TCoords* Vertices = (video::S3DVertex2TCoords*)Mesh.getMeshBuffer( 0 )->getVertices();

		for ( s32 x = startRow; x < endRow; ++x )
		{
			for ( s32 z = 0; z < TerrainData.Size; ++z )
			{
			    // get the pixel containing the information for this vertex
			    video::SColor thisPixel = structureMap->getPixel(
						inversePos - (x + dataPosition.X), z + dataPosition.Y );

			    // Load the height of the terrain from the alpha pixel
				Vertices[index].Pos.Y = (f32)thisPixel.getAlpha();

                // Load the vertex color from the color data
                Vertices[index].Color = thisPixel;

                // Set the color texture UV based on the data position
                ((video::S3DVertex2TCoords&)RenderBuffer->getVertexBuffer()[index]).TCoords2.X = (f32)(x + dataPosition.X) * imageStep;
                ((video::S3DVertex2TCoords&)RenderBuffer->getVertexBuffer()[index]).TCoords2.Y = (f32)(z + dataPosition.Y) * imageStep;
	...
	//! \param newpos: New postition of the scene node.
	void CTerrainTileSceneNode::loadPosition ( const core::vector3df& newpos, s32 startRow, s32 endRow )
	{
		TerrainData.Position = newpos;
		...
		// convert the rows into vertex indicies
		startRow *= TerrainData.Size;
		endRow *= TerrainData.Size;
	...
	//! recalculates the patch data from the new vertex information
	void CTerrainTileSceneNode::loadPatches ( void )
	{
		calculateDistanceThresholds( true );
		calculatePatchData();
		ForceRecalculation = true;
	...
	void CTerrainTileSceneNode::scaleTexture(f32 resolution, f32 resolution2)
	{
	...
	u32 CTerrainTileSceneNode::getIndex(const s32 PatchX, const s32 PatchZ,
	...
	//! smooth the terrain
	void CTerrainTileSceneNode::smoothTerrain(CDynamicMeshBuffer* mb, s32 smoothFactor)
	{
	...
	void CTerrainTileSceneNode::calculateNormals(CDynamicMeshBuffer* mb)
	{
		s32 count;
	...
	void CTerrainTileSceneNode::createPatches()
	{
	...
	void CTerrainTileSceneNode::calculatePatchData()
	{
		...
		for (s32 x = 0; x < TerrainData.PatchCount; ++x)
		{
			for (s32 z = 0; z < TerrainData.PatchCount; ++z)
			{
				...
				if (x > 0)
					...
					if ( adjacentTiles[ADJACENT_TOP] )
						TerrainData.Patches[index].Top = &adjacentTiles[ADJACENT_TOP]->TerrainData.Patches[
								(adjacentTiles[ADJACENT_TOP]->TerrainData.PatchCount - 1) *
								adjacentTiles[ADJACENT_TOP]->TerrainData.PatchCount + z];
					else
				...
				if (x < TerrainData.PatchCount - 1)
					...
					if ( adjacentTiles[ADJACENT_BOTTOM] )
						TerrainData.Patches[index].Bottom = &adjacentTiles[ADJACENT_BOTTOM]->TerrainData.Patches[z];
                    else
				...
				if (z > 0)
					...
					if ( adjacentTiles[ADJACENT_LEFT] )
						TerrainData.Patches[index].Left = &adjacentTiles[ADJACENT_LEFT]->TerrainData.Patches[
								x *	adjacentTiles[ADJACENT_LEFT]->TerrainData.PatchCount +
								adjacentTiles[ADJACENT_LEFT]->TerrainData.PatchCount - 1];
					else
				...
				if (z < TerrainData.PatchCount - 1)
					...
					if ( adjacentTiles[ADJACENT_RIGHT] )
						TerrainData.Patches[index].Right = &adjacentTiles[ADJACENT_RIGHT]->TerrainData.Patches[
								x *	adjacentTiles[ADJACENT_RIGHT]->TerrainData.PatchCount];
					else
	...
	//! used to attach an adjacent tile to this tile
	void CTerrainTileSceneNode::attachTile( CTerrainTileSceneNode *tile, unsigned int edge )
	{
		// record the adjacency
		adjacentTiles[edge] = tile;

		switch ( edge )
		{
		case ADJACENT_TOP: // Top
			for( s32 i = 0; i < TerrainData.PatchCount; ++i )
				TerrainData.Patches[i].Top =
						&tile->TerrainData.Patches[(tile->TerrainData.PatchCount-1) *
						tile->TerrainData.PatchCount + i];

            {
                // copy normals from the attached tile
                video::S3DVertex2TCoords * tarV = ((video::S3DVertex2TCoords*)RenderBuffer->getVertexBuffer().pointer());
                video::S3DVertex2TCoords * srcV = ((video::S3DVertex2TCoords*)RenderBuffer->getVertexBuffer().pointer());
                srcV += (tile->TerrainData.Size - tile->TerrainData.CalcPatchSize) * tile->TerrainData.Size;

                for ( s32 i = 0; i < TerrainData.Size; i++ )
                {
                    tarV->Normal = srcV->Normal;
		...
		case ADJACENT_BOTTOM: // Bottom
			for( s32 i = 0; i < TerrainData.PatchCount; ++i )
				TerrainData.Patches[(TerrainData.PatchCount-1) * TerrainData.PatchCount + i].Bottom =
						&tile->TerrainData.Patches[i];
			break;

		case ADJACENT_LEFT: // Left
			for( s32 i = 0; i < TerrainData.PatchCount; ++i )
				TerrainData.Patches[TerrainData.PatchCount * i].Left =
						&tile->TerrainData.Patches[tile->TerrainData.PatchCount * i +
												   tile->TerrainData.PatchCount - 1];
            {
                // copy normals from the attached tile
                video::S3DVertex2TCoords * tarV = ((video::S3DVertex2TCoords*)RenderBuffer->getVertexBuffer().pointer());
                video::S3DVertex2TCoords * srcV = ((video::S3DVertex2TCoords*)tile->RenderBuffer->getVertexBuffer().pointer());
                srcV += tile->TerrainData.Size - tile->TerrainData.CalcPatchSize;

                for ( s32 i = 0; i < TerrainData.Size; i++ )
                {
                    tarV->Normal = srcV->Normal;

                    tarV += TerrainData.Size;
                    srcV += TerrainData.Size;
                }
            }
			break;

		case ADJACENT_RIGHT: // Right
			for( s32 i = 0; i < TerrainData.PatchCount; ++i )
				TerrainData.Patches[TerrainData.PatchCount * i + TerrainData.PatchCount-1].Right =
						&tile->TerrainData.Patches[tile->TerrainData.PatchCount * i];
			break;
		}
	}
	...
	void CTerrainTileSceneNode::calculateDistanceThresholds(bool scalechanged)
	{
	...
	void CTerrainTileSceneNode::setCurrentLODOfPatches(s32 lod)
	{
		const s32 count = TerrainData.PatchCount * TerrainData.PatchCount;
	...
	void CTerrainTileSceneNode::setCurrentLODOfPatches(const core::array<s32>& lodarray)
	{
		const s32 count = TerrainData.PatchCount * TerrainData.PatchCount;
		for (s32 i=0; i<count; ++i)
	...
	//! Gets the height
	f32 CTerrainTileSceneNode::getHeight(f32 x, f32 z) const
	{
		if (!Mesh.getMeshBufferCount())
			return 0;
	...
	void CTerrainTileSceneNode::serializeAttributes(io::IAttributes* out,
	...
	void CTerrainTileSceneNode::deserializeAttributes(io::IAttributes* in,
	...
	ISceneNode* CTerrainTileSceneNode::clone(ISceneNode* newParent, ISceneManager* newManager)
	{
		if (!newParent)
			newParent = Parent;
		if (!newManager)
		...
		CTerrainTileSceneNode* nb = new CTerrainTileSceneNode(
