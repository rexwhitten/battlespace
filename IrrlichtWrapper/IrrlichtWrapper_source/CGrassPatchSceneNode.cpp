...
CGrassPatchSceneNode::CGrassPatchSceneNode(
		...
		s32 patchScale, f32 grassScale, u32 maxDensity, core::position2d<s32> dataPosition,
		char *filepath, video::IImage* heightMap, video::IImage* colourMap,
		video::IImage* grassMap, scene::IWindGenerator *windgen, bool grassCrossed) :

		 ISceneNode((ITerrainSceneNode*)terrain, mgr, -1),
		 GRASS_PATCH_SIZE( patchScale ),
		 grassScale( grassScale ),
	   	 ...
	   	 redrawnextloop(true),
	   	 fpsLock(30),
		 crossed(grassCrossed)
{
	#ifdef _DEBUG
	setDebugName("CGrassPatchSceneNode");
	#endif

    // set the maximum draw distance for this patch
    DrawDistanceSQ = GRASS_PATCH_SIZE*1.5f*GRASS_PATCH_SIZE*1.5f;
...
bool CGrassPatchSceneNode::Create( core::position2d<s32> dataPosition )
{
	f32 x, z;
	f32 rotation, lastrotation = 0.0f;
	core::dimension2d<f32> size;

	srand((100 * gridpos.X) + gridpos.Z);

    // particle count
    s32 count = (s32)MaxDensity; // lots. we dont need them all
    ...
    {
		// if we are drawing crossed particles and this is an odd numbered particle
		if (( crossed ) && ( i % 2 == 1 ))
		{
	        // duplicate properties from previous particle
			Particles[i].color = Particles[i-1].color;
			Particles[i].startColor = Particles[i-1].startColor;
			Particles[i].pos = Particles[i-1].pos;
			Particles[i].sprite = Particles[i-1].sprite;
			Particles[i].flex = Particles[i-1].flex;

			// rotate this particle by 90 degrees to form a cross
			rotation = lastrotation + 90.0f;
		}
		else
		{
	        // get random x and z axis
			x = (rand() % (GRASS_PATCH_SIZE*10) ) / 10.0f;
			z = (rand() % (GRASS_PATCH_SIZE*10) ) / 10.0f;
			...
			if (( crossed ) && ( i % 2 == 1 ))
			{
				Particles[i].sprite.Width  = Particles[i-1].sprite.Width;
			}
			else
			{
			...
			if ( Terrain )
			{
				xz.X = p.X/Terrain->getScale().X;
				xz.Y = 0.0f;
				xz.Z = p.Z/Terrain->getScale().Z;
				...
				cDensity  = TerrainGrassMap->getPixel(x1+dataPosition.X,z1+dataPosition.Y);
				...
				Particles[i].color	  =  TerrainColourMap->getPixel(x1+dataPosition.X,z1+dataPosition.Y);
				Particles[i].startColor =  TerrainColourMap->getPixel(x1+dataPosition.X,z1+dataPosition.Y);

				height = Terrain->getHeight(p.X, p.Z);
			}
			size = core::dimension2d<f32> ((f32)(rand() % 50 + 20) * grassScale, 100.0f * grassScale);

			size.Height *= (f32)cDensity.getGreen()/200.0f;

			Particles[i].flex  = size.Height / 120.0f;
			Particles[i].pos.Y = height + (size.Height*0.5f);

	        lastrotation = rotation = (rand() % 3600) / 10.0f;
		}
