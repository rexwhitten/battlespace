...
CSkyDomeColorSceneNode::CSkyDomeColorSceneNode(video::ITexture* sky, u32 horiRes, u32 vertRes,
			f64 texturePercentage, f64 spherePercentage, f64 domeRadius, ISceneNode* parent, ISceneManager* mgr, s32 id)
			: ISceneNode(parent, mgr, id)
{
	#ifdef _DEBUG
	setDebugName("CSkyDomeColorSceneNode");
	#endif

	horizontalRes = horiRes;
	verticalRes = vertRes;

	radius = domeRadius; // 1000.0; /* Adjust this to get more or less perspective distorsion. */
...
//! renders the node.
void CSkyDomeColorSceneNode::render()
{
...
const core::aabbox3d<f32>& CSkyDomeColorSceneNode::getBoundingBox() const
{
	return Buffer.BoundingBox;
}


void CSkyDomeColorSceneNode::OnRegisterSceneNode()
{
	if (IsVisible)
	{
...
video::SMaterial& CSkyDomeColorSceneNode::getMaterial(u32 i)
{
...
u32 CSkyDomeColorSceneNode::getMaterialCount() const
{
	return 1;
}
