...
void CBatchingMesh::setHardwareMappingHint(irr::scene::E_HARDWARE_MAPPING newMappingHint, irr::scene::E_BUFFER_TYPE buffer)
{
	u32 i;
	for (i=0; i<DestBuffers.size(); ++i)
	{
		DestBuffers[i].Buffer->setHardwareMappingHint(newMappingHint, buffer);
	}
}

void CBatchingMesh::setDirty(E_BUFFER_TYPE buffer)
{
	u32 i;
	for (i=0; i<DestBuffers.size(); ++i)
	{
		DestBuffers[i].Buffer->setDirty(buffer);
